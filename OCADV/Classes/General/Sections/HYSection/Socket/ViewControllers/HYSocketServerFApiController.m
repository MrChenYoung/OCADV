...
@interface HYSocketServerFApiController ()

// 服务端socket
@property (nonatomic, strong) HYSocketFoundationServer *serverSocket;

@end

@implementation HYSocketServerFApiController
...
{
    self.serverSocket = [[HYSocketFoundationServer alloc]init];
    
    __weak typeof(self) weakSelf = self;
    // 有新的客户端连接
    self.serverSocket.newClientConnectCallBack = ^(HYSocketClientModel *newClientModel, NSInteger connectedCount) {
    ...
    // 有客户端断开连接
    self.serverSocket.clientDisconnectionCallBack = ^(HYSocketClientModel * _Nonnull newClientModel, NSInteger connectedCount) {
    ...
    // 收到客户端发送的消息
    self.serverSocket.recMessageCallBack = ^(HYSocketClientModel * _Nonnull clientModel, NSString * _Nonnull message) {
