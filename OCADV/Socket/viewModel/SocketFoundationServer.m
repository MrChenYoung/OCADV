...
@interface SocketFoundationServer ()

// 服务器socket
@property (nonatomic, assign) int serverSocket;

// 已经连接的客户端
@property (nonatomic, strong) NSMutableArray<SocketClientModel *> *connectedClients;
...
@implementation SocketFoundationServer
...
- (void)stopListen
{
    ...
    // 断开客户端socket
    for (SocketClientModel *clientSocket in self.connectedClients) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        do {
            ...
            if (recLength > 0) {
                ...
                // 获取客户端socket的ip和端口
                SocketClientModel *currentClentModel = nil;
                for (SocketClientModel *clientModel in weakSelf.connectedClients) {
                ...
                for (SocketClientModel *model in weakSelf.connectedClients) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    for (SocketClientModel *client in removeClients) {
                        // 移除客户端
...
- (void)sendBroadcastMessage:(NSString *)message
{
    ...
    for (SocketClientModel *client in self.connectedClients) {
