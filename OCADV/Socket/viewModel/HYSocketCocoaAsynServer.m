...
@interface HYSocketCocoaAsynServer ()<GCDAsyncSocketDelegate>

// 是否正在监听
@property (nonatomic, assign) BOOL isListening;

// 已经连接的客户端socket模型
@property (nonatomic, strong) NSMutableArray<HYSocketClientModel *> *clientSocketModels;
...
@implementation HYSocketCocoaAsynServer
...
- (void)sendBroadcastMessage:(NSString *)message
{
    ...
    [self.clientSocketModels enumerateObjectsUsingBlock:^(HYSocketClientModel *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)socket:(GCDAsyncSocket *)sock didAcceptNewSocket:(GCDAsyncSocket *)newSocket
{
    ...
    HYSocketClientModel *model = [[HYSocketClientModel alloc]init];
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err
{
    if (sock == self.serverSocket) {
        ...
        // 断开已经连接的客户端
        for (HYSocketClientModel *model in self.clientSocketModels) {
    ...
    for (HYSocketClientModel *client in self.clientSocketModels) {
        if (client.socket == sock) {
            [removeClient addObject:client];
        }
    }
    for (HYSocketClientModel *model in removeClient) {
        [self.clientSocketModels removeObject:model];
    }
    
    // 更新界面
    dispatch_async(dispatch_get_main_queue(), ^{
        for (HYSocketClientModel *disconnectClient in removeClient) {
