...
@interface SocketClientView()<UITextViewDelegate>
...
@implementation SocketClientView
...
- (void)createSubViews
{
    ...
    [SocketClientView createTipLabel:ipPortFrame text:@"连接到的ip和端口:" superView:self];
    
    // 服务端ip
    CGFloat totalW = ScWidth - 30;
    CGFloat height = 30;
    CGRect ipFrame = CGRectMake(10, CGRectGetMaxY(ipPortFrame) + 5, totalW * 0.8, height);
    _ipTextView = [SocketClientView createInputTextFiled:ipFrame placeHold:@"服务端ip" superView:self];
    ...
    _portTextView = [SocketClientView createInputTextFiled:portFrame placeHold:@"端口" superView:self];
    ...
    UILabel *recMsgTipLabel = [SocketClientView createTipLabel:recMsgTipFrame text:@"接收到的消息:" superView:self];
    CGRect recTextFrame = CGRectMake(10, CGRectGetMaxY(recMsgTipLabel.frame) + 5, ScWidth - 20, ScHeight * 0.2);
    _recTextView = [SocketClientView createInputTextFiled:recTextFrame placeHold:@"暂无接收到任何消息" superView:self];
    _recTextView.editable = NO;
    
    // 发送消息
    CGRect sendMsgTipFrame = CGRectMake(10, CGRectGetMaxY(recTextFrame) + 10, ScWidth - 20, 30);
    UILabel *sendMsgTipLabel = [SocketClientView createTipLabel:sendMsgTipFrame text:@"发送消息:" superView:self];
    CGRect messageFrame = CGRectMake(10, CGRectGetMaxY(sendMsgTipLabel.frame) + 5, ScWidth - 20, ScHeight * 0.2);
    _messageTextView = [SocketClientView createInputTextFiled:messageFrame placeHold:@"输入要发送的消息" superView:self];
