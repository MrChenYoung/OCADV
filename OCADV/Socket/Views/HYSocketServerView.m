...
@implementation HYSocketServerView
...
- (void)createSubViews
{
    ...
    [HYSocketClientView createTipLabel:ipPortFrame text:@"监听的端口:" superView:self.scrollView];
    
    // 监听的端口号
    CGRect portFrame = CGRectMake(10, CGRectGetMaxY(ipPortFrame) + 5, ScWidth - 20, 30);
    _portTextView = [HYSocketClientView createInputTextFiled:portFrame placeHold:@"输入要监听的端口" superView:self.scrollView];
    ...
    _onlineClientsLabel = [HYSocketClientView createTipLabel:onlineClientsTipFrame text:@"已连接的客户端(0):" superView:self.scrollView];
    CGRect onlineClientsFrame = CGRectMake(10, CGRectGetMaxY(onlineClientsTipFrame) + 5, ScWidth - 20, ScHeight * 0.2);
    _onlineClientsTextView = [HYSocketClientView createInputTextFiled:onlineClientsFrame placeHold:@"暂无客户端连接" superView:self.scrollView];
    ...
    [HYSocketClientView createTipLabel:recMessageTipFrame text:@"接收到的消息:" superView:self.scrollView];
    CGRect recMessageFrame = CGRectMake(10, CGRectGetMaxY(recMessageTipFrame) + 5, ScWidth - 20, ScHeight * 0.2);
    _recMessageTextView = [HYSocketClientView createInputTextFiled:recMessageFrame placeHold:@"暂无客户端消息" superView:self.scrollView];
    _recMessageTextView.editable = NO;
    
    // 发送消息给客户端
    CGRect sendMessageTipFrame = CGRectMake(10, CGRectGetMaxY(recMessageFrame) + 10, ScWidth - 20, 30);
    [HYSocketClientView createTipLabel:sendMessageTipFrame text:@"发送消息给客户端:" superView:self.scrollView];
    CGRect sendMessageFrame = CGRectMake(10, CGRectGetMaxY(sendMessageTipFrame) + 5, ScWidth - 20, ScHeight * 0.2);
    _sendMessageTextView = [HYSocketClientView createInputTextFiled:sendMessageFrame placeHold:@"输入要发送给客户端的消息" superView:self.scrollView];
